...
async function testPaymentSystem() {
    ...
    try {
        ...
        await client.query("UPDATE accounts SET balance = 0 WHERE user_id IN ($1, $2)", [user1, user2]);
        ...
        await paymentController.addFunds({ user: { userId: user1 }, body: { amount: 1000 } }, {
            json: (data) => console.log('AddFunds Response:', data),
            status: (code) => ({ json: (data) => console.log(`AddFunds Error (${code}):`, data) })
        });

        const bal1 = await client.query("SELECT balance FROM accounts WHERE user_id = $1", [user1]);
        ...
        try {
            ...
            await paymentController.processTransfer(user1, user2, 500, 'test', 123, txClient);
        ...
        const bal1_after = await client.query("SELECT balance FROM accounts WHERE user_id = $1", [user1]);
        const bal2_after = await client.query("SELECT balance FROM accounts WHERE user_id = $1", [user2]);
        console.log(`User A Balance: ${bal1_after.rows[0].balance} (Expected 500.00)`);
        console.log(`User B Balance: ${bal2_after.rows[0].balance} (Expected 500.00)`);
        ...
        try {
            ...
            await paymentController.processTransfer(user1, user2, 1000, 'test_fail', 124, txClient2);
