...
export default function PatientPrescriptions({ userId }) {
  ...
  return (
    <div className="space-y-8 animate-fade-in p-2 md:p-4">
      {/* Ongoing Medicines */}
      <div>
        <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2" style={{ marginLeft: '5px' }}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {ongoing.map((p) => (
              <div
                ...
                className="bg-card p-6 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 hover:shadow-md transition-all relative overflow-hidden group"
              >
                ...
                <h3 className="font-bold text-slate-900 dark:text-white text-lg group-hover:text-emerald-300 transition-colors" style={{ marginLeft: '20px' }}>
                  {p.medicine_name}
                </h3>
                <div className="text-sm text-slate-400 mt-4 mb-4" style={{ marginLeft: '20px', marginTop: '20px', marginBottom: '30px' }}>
                  <span className="font-semibold bg-slate-200 dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 mr-2" style={{ marginRight: '15px' }}>
                    {p.dosage}
                  </span>
                  {p.instructions}
                </div>
                <div className="flex items-center justify-between text-sm text-slate-500 pt-3 border-t border-slate-200/50 dark:border-slate-700/50" style={{ marginLeft: '20px', marginRight: '10px', marginTop: '15px', marginBottom: '15px' }}>
        ...
        {past.length === 0 ? (
          ...
          <div className="grid gap-3 opacity-75">
            {past.map((p) => (
              ...
              >
                <div>
                  <h3 className="font-semibold text-slate-700 dark:text-slate-300">{p.medicine_name}</h3>
