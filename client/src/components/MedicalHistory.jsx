  ...
  const derivedDoctors = useMemo(() => {
    ...
    if (currentUser?.role === "doctor" && currentUser?.user_id) {
      ...
      if (!seen.has(id)) {
        ...
        list.push({
          user_id: id,
          full_name: currentUser.full_name || "Current Doctor",
        });
      }
    }
  ...
  return (
    <div className="min-h-[calc(100vh-140px)] animate-fade-in">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex justify-between items-center gap-4 sticky top-4 z-10 bg-white/90 dark:bg-slate-900/80 backdrop-blur border border-slate-200/60 dark:border-slate-700/60 rounded-2xl px-5 py-3 shadow-md shadow-slate-200/40 dark:shadow-black/20">
          <div>
          ...
          <button
            onClick={() => openModal()}
            className="inline-flex items-center justify-center h-10 gap-2 px-5 text-sm font-semibold bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl shadow-md shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:-translate-y-0.5 transition-all duration-300"
          >
            <svg
              className="w-5 h-5"
              fill="none"
        ...
        <div className="grid grid-cols-1 gap-6">
          {medicalHistory?.map((history, idx) => (
            <div
              ...
              className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg shadow-slate-200/40 dark:shadow-black/20 overflow-hidden hover:-translate-y-1 hover:shadow-xl transition-all duration-300 animate-fade-in"
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-slate-500 text-sm">
                          ...
                          Dr.{" "}
                          {history.doctor_name ||
                            doctorLookup[String(history.doctor_id)] ||
                            "Unknown"}
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-200 rounded-xl font-semibold text-sm">
                        <svg
                          className="w-4 h-4"
                          fill="none"
                          stroke="currentColor"
