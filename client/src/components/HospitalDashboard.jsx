...
export default function HospitalDashboard() {
  ...
  const myHospital = hospitals?.[0]; // Assuming admin manages the first hospital for demo
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in">
      <div className="bg-emerald-600 rounded-3xl p-10 text-white shadow-xl shadow-emerald-200">
        ...
        <p className="text-emerald-100 text-lg opacity-90">{myHospital?.name || "General Hospital"}</p>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10">
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
            <p className="text-emerald-100 text-sm font-medium mb-1 uppercase tracking-wider">Total Doctors</p>
            <p className="text-4xl font-bold text-white">{totalDoctors}</p>
          </div>
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
            <p className="text-emerald-100 text-sm font-medium mb-1 uppercase tracking-wider">Total Patients</p>
            <p className="text-4xl font-bold text-white">{totalPatients}</p>
          </div>
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
            <p className="text-emerald-100 text-sm font-medium mb-1 uppercase tracking-wider">Departments</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button className="p-4 rounded-xl bg-slate-50 hover:bg-emerald-50 hover:text-emerald-600 transition-colors text-left group">
              ...
              <span className="font-bold text-sm block">Manage Departments</span>
            </button>
            <button className="p-4 rounded-xl bg-slate-50 hover:bg-emerald-50 hover:text-emerald-600 transition-colors text-left group">
              <span className="block text-2xl mb-2 group-hover:scale-110 transition-transform">📊</span>
          ...
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex gap-4 items-start pb-4 border-b border-slate-50 last:border-0 last:pb-0">
                ...
                <div>
                  <p className="text-sm font-medium text-slate-800">New appointment booked for Dr. Smith</p>
