...
export default function StaffManagement() {
  ...
  const updateFeeMutation = useMutation({
    mutationFn: ({ doctorId, fee }) => hospitalsAPI.updateDoctorFee(doctorId, fee),
    onSuccess: () => {
      queryClient.invalidateQueries(["my-hospital-doctors"]);
      ...
      <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-md border border-slate-200/50 dark:border-slate-700/50 rounded-xl overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-lg">No doctors added yet</p>
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  {editingDoctor === doctor.user_id ? (
                    ...
                    >
                      <span className="font-semibold">৳{doctor.consultation_fee || 0}</span>
                  ...
                  {/* Remove Button */}
                  <button
                    onClick={() => handleRemoveDoctor(doctor.user_id, doctor.full_name)}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] overflow-hidden">
            ...
            <div className="max-h-64 overflow-y-auto">
              {filteredAvailableDoctors.length === 0 ? (
                <p className="text-center text-slate-500 py-8">No available doctors found</p>
              ) : (
            ...
            {selectedDoctor && (
              <div className="p-6 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
                ...
                >
                  {addDoctorMutation.isPending ? "Adding..." : `Add Dr. ${selectedDoctor.full_name}`}
                </button>
              </div>
            )}
          </div>
