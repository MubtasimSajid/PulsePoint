...
export default function Wallet() {
    ...
    if (isLoading) return <div className="p-8 text-center text-slate-500">Loading wallet...</div>;
            ...
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl p-8 text-white shadow-xl shadow-slate-200/50">
                ...
                <form onSubmit={handleAddFunds} className="flex gap-4 items-end">
                    <div className="flex-1">
                        <label className="block text-sm text-slate-400 mb-2">Add Funds</label>
                        ...
                        wallet?.transactions?.map((tx) => (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    <div
                                        className={`w-10 h-10 rounded-full flex items-center justify-center ${tx.type === "deposit" || tx.type === "refund"
                                    ...
                                    <div>
                                        <p className="font-medium text-slate-900 capitalize">{tx.description || tx.type}</p>
                                ...
                                <div className="text-right">
                                    <p
                                        className={`font-bold ${tx.type === "deposit" || tx.type === "refund"
                                                ? "text-emerald-600"
                                                : "text-slate-900"
                                            }`}
                                    >
                                        {tx.type === "deposit" || tx.type === "refund" ? "+" : "-"} {wallet.currency}{" "}
                                        {tx.amount}
                                    </p>
