  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                 <label className="block text-sm text-slate-400 mb-1">File Link (URL)</label>
                 <input
                   type="url"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {records?.map((record) => (
          <div key={record.record_id} className="bg-slate-800/50 border border-slate-700 rounded-xl p-5 hover:border-emerald-500/30 transition group relative">
            <button
               onClick={() => deleteMutation.mutate(record.record_id)}
               className="absolute top-4 right-4 text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition"
               title="Delete Record"
            >
              🗑️
            </button>
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-400 uppercase tracking-wide font-bold mt-1">
                  {record.record_type} • {new Date(record.record_date).toLocaleDateString()}
                </p>
                {record.description && (
                  <p className="text-slate-400 text-sm mt-2 line-clamp-2">{record.description}</p>
                )}
                {record.file_url && (
                  <a 
